  ...
  try {
    ...
    db.exec(`
      CREATE TABLE IF NOT EXISTS turn_file_changes (
        ...
        python_changed INTEGER NOT NULL DEFAULT 0,
        updated_at TEXT NOT NULL
      );

      CREATE TABLE IF NOT EXISTS turn_python_projects (
...
function deleteTurns(db, turnIds) {
  ...
  db.prepare(
    `DELETE FROM turn_python_projects WHERE turn_id IN (${placeholders})`,
  ).run(...turnIds);
  db.prepare(
...
function maybePruneState(db, nowIso) {
  ...
  const pruneIntervalMinutes = envPositiveInt(
    "PYTHON_HOOKS_STATE_PRUNE_INTERVAL_MINUTES",
  ...
  const retentionHours = envPositiveInt(
    "PYTHON_HOOKS_STATE_RETENTION_HOURS",
    DEFAULT_RETENTION_HOURS,
  );
  const maxTurns = envPositiveInt(
    "PYTHON_HOOKS_STATE_MAX_TURNS",
    DEFAULT_MAX_TURNS,
  );
...
export function markPythonChanged(turnId, projectRoots = []) {
  ...
  try {
    db.prepare(`
      INSERT INTO turn_file_changes (turn_id, python_changed, updated_at)
      VALUES (?, 1, ?)
    ...
    const insertProject = db.prepare(`
      INSERT INTO turn_python_projects (turn_id, project_root, updated_at)
      VALUES (?, ?, ?)
...
export function getPythonTurnState(turnId) {
  if (typeof turnId !== "string" || turnId.length === 0) {
    return null;
  }
  ...
  try {
    const row = db
      .prepare(
        "SELECT python_changed FROM turn_file_changes WHERE turn_id = ? LIMIT 1",
      )
      .get(turnId);
    if (!row) {
      return { pythonChanged: false, projectRoots: [] };
    }
    ...
    return {
      pythonChanged: Boolean(row.python_changed),
      projectRoots,
    };
  } catch {
    return null;
