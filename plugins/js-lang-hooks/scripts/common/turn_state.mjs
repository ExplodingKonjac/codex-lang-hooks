...
function deleteTurns(db, turnIds) {
  ...
  db.prepare(
    `DELETE FROM turn_js_projects WHERE turn_id IN (${placeholders})`,
  ).run(...turnIds);
  db.prepare(
    `DELETE FROM turn_js_files WHERE turn_id IN (${placeholders})`,
  ).run(...turnIds);
  db.prepare(
...
function maybePruneState(db, nowIso) {
  ...
  const pruneIntervalMinutes = envPositiveInt(
    "JS_HOOKS_STATE_PRUNE_INTERVAL_MINUTES",
  ...
  const retentionHours = envPositiveInt(
    "JS_HOOKS_STATE_RETENTION_HOURS",
    DEFAULT_RETENTION_HOURS,
  );
  const maxTurns = envPositiveInt(
    "JS_HOOKS_STATE_MAX_TURNS",
    DEFAULT_MAX_TURNS,
  );
