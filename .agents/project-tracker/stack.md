...
| JavaScript/TypeScript hook state DB | SQLite file at `${PLUGIN_DATA}/js-lang-hooks.sqlite3` | Records whether a turn changed JS/TS files/config and which JS/TS project roots need Stop checks. |
...
- JavaScript/TypeScript hook behavior, discovered `package.json` parsing, and failed-command output size are configurable through local `JS_HOOKS_*` environment variables; no new runtime dependency is introduced for configuration.
